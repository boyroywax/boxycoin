...
The headless daemon `boxycoind` has the JSON-RPC API enabled by default, the GUI
`boxycoin-qt` has it disabled by default. This can be changed with the `-server`
