# PSBT Howto for Boxycoin Core

Since Boxycoin Core 2.0.0, an RPC interface exists for Partially Signed Boxycoin
...
PSBT is an interchange format for Boxycoin transactions that are not fully signed
...
Overall, the construction of a fully signed Boxycoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Boxycoin transaction (in network format)
...
#### Multisig with multiple Boxycoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Boxycoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `cranepaboxycoinis used.
